...
public protocol SessionCallback {
    ...
    func onError(_ errorMessage: CashSDKError?)
}

public enum CashSDKUrl: String {
    case Staging = "https://secure.just.cash"
    case Production = "https://api-prd.just.cash"
}

open class CashSDK: CashSDKProtocol  {
    ...
    public init(url: CashSDKUrl = .Production) {
    ...
    func decode<T:Decodable>(_ data: Data, completion: @escaping (T) -> ()) {
        ...
        if let error = object.error {
            if (Int(error.code) == CashSDKErrorCode.sessionTimeout.rawValue) {
                                ...
                                if (error == nil) {
                                    self?.decode(data!) { (response: CashCodeResponse) in
                                        if response.error != nil {
                                            if (Int(response.error!.code) == CashSDKErrorCode.sessionTimeout.rawValue) {
    ...
    private func handleSessionExpiry(_ error: CashSDKError, completion: @escaping (()-> Void)) {
